...
# url = "https://sppzaaipmgcsxsytoeqb.supabase.co"
# key ="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNwcHphYWlwbWdjc3hzeXRvZXFiIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDQxNzg3MDcsImV4cCI6MjA1OTc1NDcwN30.BcQ0p5swk3YdjIshSR8UqDw_Uzs5S1YmakAyI6yJ8JQ"
...
#     """Create a side panel with face information that matches video height"""
...
#         except Exception as e:
#             print(f"Frame combining error: {e}")
...
#                         c1=emotion in ['Angry','Fear', 'Sad']
#                         c2=(supabase.table("User").select("*").eq("userName", "suganth").execute()).data[0]['emotion']
#                         print(f"Name : {name}|| Previous Emotion : {c2} || Current Emotion : {emotion} ")
#                         if(c1 and emotion != c2):
#                             user_res = supabase.table("User").select("*").eq("userName", name).limit(1).execute()
#                             if user_res.data:
#                                 user_id = user_res.data[0]["id"]
#                                 print("User found:", user_res.data)
#                                 update_res = supabase.table("User").update({"emotion": emotion}).eq("id", user_id).execute()
#                                 print("Emotion updated:", update_res.data)
#                             else:
#                                 print("User not found.")
#                     else:
#                         is_new_face = True
#                         for saved_encoding in saved_unknown_encodings:
#                             if face_recognition.compare_faces([saved_encoding], face_encoding, 
#                                                            tolerance=UNKNOWN_TOLERANCE)[0]:
#                                 is_new_face = False
#                                 break
#                         if is_new_face:
#                             top_full, right_full, bottom_full, left_full = [int(coord/RESIZE_SCALE) for coord in (top, right, bottom, left)]
...
def load_known_faces_from_supabase():
    ...
    response = supabase.table("Family").select("name,image").execute()
                ...
                for face_encoding, (top, right, bottom, left) in zip(face_encodings, face_locations):
                    ...
                    if len(face_distances) > 0 and matches[np.argmin(face_distances)]:
                        ...
                        if emotion in ['Angry', 'Fear', 'Sad']:
                            user_res = supabase.table("Family").select("*").eq("name", name).limit(1).execute()
                            if user_res.data:
                                user_id = user_res.data[0]["id"]
                                current_emotion = user_res.data[0].get("emotion")
                                if current_emotion != emotion:
                                    update_res = supabase.table("Family").update({"emotion": emotion}).eq("id", user_id).execute()
